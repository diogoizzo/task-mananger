...
interface btnTarefaInboxProps {
   text: string;
   onClick: MouseEventHandler;
   className?: string;
   manage?: boolean;
}
export default function BtnTarefaInbox({
   onClick,
   text,
   className,
   manage
   ...
   return (
      <button
         onClick={onClick}
         className={`flex flex-wrap items-center justify-center py-3 px-4 w-full text-base text-white font-medium transition-colors bg-indigo-600 hover:bg-indigo-500 rounded-md shadow-button ${className}`}
      >
         {manage ? (
            <svg
         ...
         <span className="text-[#e0e7ff]">{text}</span>
      </button>
   );
}
